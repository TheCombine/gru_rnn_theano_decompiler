...
# 05/10 improvinu TheanoGRU2 pridedamas initialstate prie forwardprop
...
class GRU(object):
    ...
    def __theano_build__(self):
        ...
        self.cost = theano.function([SeqIndex, InitState, SeqIndexTruth], cost)
        ...
        self.sgd_step = theano.function(
            [SeqIndex, InitState, SeqIndexTruth],
            [], 
        ...
        for epoch in xrange(nepoch):
            for SeqIndexObj, SeqIndexObjTruth, SeqIndexSrc, SeqIndexSrcTruth in zip(SeqSeqIndexObj, SeqSeqIndexObjTruth, SeqSeqIndexSrc, SeqSeqIndexSrcTruth):
                if evaluate_loss_every > 0 and num_samples_seen % evaluate_loss_every == 0:
                    ...
                    curr_loss = self.encoder.calculate_loss(SeqSeqIndexObj, np.zeros(self.encoder.hidden_dim), SeqSeqIndexObjTruth)
                    ...
                    # cia blogai kad init state zeros paduodu!!!!
                    # cia blogai kad init state zeros paduodu!!!!
                    # cia blogai kad init state zeros paduodu!!!!
                    curr_loss = self.decoder.calculate_loss(SeqSeqIndexSrc, np.zeros(self.encoder.hidden_dim), SeqSeqIndexSrcTruth)
                ...
                self.encoder.sgd_step(SeqIndexObj, np.zeros(self.encoder.hidden_dim), SeqIndexObjTruth)
                # koki initial state i decoder paduoti???
                # koki initial state i decoder paduoti???
                # koki initial state i decoder paduoti???
                self.decoder.sgd_step(SeqIndexSrc, np.zeros(self.encoder.hidden_dim), SeqIndexSrcTruth)
                num_samples_seen += 1
